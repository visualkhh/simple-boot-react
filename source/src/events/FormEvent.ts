...
export namespace FormEvent {
  ...
  export const preventDefaultObject = <T extends unknown>(
    ...
    config?: {
      ...
      forceData?: any;
      excludes?: string[];
      fieldConverts?: { name: string; convert: (e: HTMLInputElement, value: FormDataEntryValue) => any }[];
    ...
    return (e: React.FormEvent<HTMLFormElement> | React.ChangeEvent<HTMLFormElement>) => {
      ...
      const setting = (value: FormDataEntryValue, key: string, bowl: any, originName: string) => {
        ...
        // @ts-ignore
        const keys = Array.from(('keys' in formData) ? formData.keys() : []);
        const allElement = (Array.from(form.querySelectorAll(`[name="${key}"]`)) ?? []) as HTMLElement[];
        ...
        if (convert) {
          if (currentElement) {
            value = convert.convert(currentElement as HTMLInputElement, value);
          }
        }
        // console.log('---->ke', keys, key, value);
        if (
          bowl[key] === undefined &&
          (allElement.length > 1 ||
            keys.filter(it => it === key).length > 1 ||
            (allElement.length === 1 && (currentElement as HTMLInputElement).multiple))
        ) {
          bowl[key] = [];
        }
